...
const PLASTIC_MIGRATION_HISTORICAL_PAYMENT_APPLIED_RP=57131000;
const PLASTIC_MIGRATION_HISTORICAL_REMAINING_PAYABLE_RP=19094000;
...
if(cmd==='UPDATE_SALE'){
  ...
  const normalized=lines.map((r:any)=>{
    ...
    /* RKN_PLASTIC_EXPLICIT_SALE_PRICE_V2S */
    if(price<=0){
    ...
    if(price<=0)throw Error('PLASTIC_SALE_PRICE_REQUIRED');
